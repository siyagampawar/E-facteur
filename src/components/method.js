...
const getlatLng = (pincode) => {
  return codes[pincode];
};

const calculateDistance = (toPincode, fromPincode) => {
  let distance = 1;
  const doesExist = manualDistance[toPincode];
  if (doesExist && doesExist[fromPincode]) {
    distance = doesExist[fromPincode];
  } else {
    const toCoords = getlatLng(toPincode);
    const fromCoords = getlatLng(fromPincode);
...
const calculatePincodeDistance = (toPincode, fromPincode) => {
  return calculateDistance(toPincode, fromPincode);
};

export default calculatePincodeDistance;